...
async function runChain(extraArgs) {
  const candidates = [
    path4.resolve(__dirname, "../../futurchain/target/release/futurchain"),
    path4.resolve(__dirname, "../../../futurchain/target/release/futurchain"),
    "futurchain"
    // in PATH
  ];
  ...
  if (!binary) {
    const chainDir = candidates.map((b) => path4.dirname(path4.dirname(path4.dirname(b)))).find((d) => require("fs").existsSync(path4.join(d, "Cargo.toml")));
    if (!chainDir) {
  ...
  const child = spawn2(binary, extraArgs, { stdio: "inherit" });
