...
function unify(a, b, metavars) {
    ...
    if (!unifyInto(a, b, metavars, subst, errorRef)) {
        return { subst, error: errorRef.error || { expected: (0, term_1.termToString)(b), actual: (0, term_1.termToString)(a) } };
    }
    return { subst };
}
function unifyInto(a, b, metavars, subst, errorRef) {
    ...
    if (a.kind === 'var' && metavars.has(a.name)) {
        ...
        if (occursIn(a.name, b)) {
            errorRef.error = { expected: `No occurs-check cycle for ${a.name}`, actual: (0, term_1.termToString)(b) };
            return false;
        }
        subst.set(a.name, b);
        return true;
    ...
    if (b.kind === 'var' && metavars.has(b.name)) {
        if (occursIn(b.name, a)) {
            errorRef.error = { expected: `No occurs-check cycle for ${b.name}`, actual: (0, term_1.termToString)(a) };
            return false;
        }
        subst.set(b.name, a);
        return true;
    ...
    if (a.kind !== b.kind) {
        errorRef.error = { expected: (0, term_1.termToString)(b), actual: (0, term_1.termToString)(a) };
        return false;
    }
    switch (a.kind) {
        ...
        case 'implies': {
            const bb = b;
            return unifyInto(a.left, bb.left, metavars, subst, errorRef)
                && unifyInto(a.right, bb.right, metavars, subst, errorRef);
        }
        case 'not': {
        ...
        case 'eq': {
            ...
            if (unifyInto(aa.left, bb.left, metavars, subst, errorRef) && unifyInto(aa.right, bb.right, metavars, subst, errorRef))
                return true;
