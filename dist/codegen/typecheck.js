...
exports.typecheckExecutableProgram = typecheckExecutableProgram;
...
function inferBodyReturnTypes(nodes, env) {
    ...
    for (const node of nodes) {
        if (node.type === 'SetVar' && node.value !== null) {
            ...
            if (rawReturn)
                returns.push(inferRawType(rawReturn[1], env));
        }
    }
    return returns;
}
...
function bindPattern(pattern, scrutineeType, env) {
    if (pattern.kind === 'list_cons') {
        ...
        if (pattern.head !== '_')
            env.vars.set(pattern.head, elementType);
...
function inferExprType(expr, env) {
    switch (expr.type) {
        ...
        case 'Fold': {
            ...
            if (!sequenceType.startsWith('List('))
                return 'Any';
            if (!fnType.includes('->'))
                return initType;
            return initType;
...
function inferValueType(value, env) {
    try {
        return inferExprType((0, expr_1.parseExpr)(value), env);
    }
    catch {
...
function inferRawType(value, env) {
    ...
    if (trimmed === '[]')
        return 'List(Any)';
    if (/^\[.*\]$/.test(trimmed))
        return inferListType(trimmed, env);
    if (env.vars.has(trimmed))
        return env.vars.get(trimmed);
    ...
    if (/&&|\|\|/.test(trimmed))
        return 'Bool';
    if (/[+\-*/]/.test(trimmed))
        return inferNumericType(trimmed, env);
    return 'Any';
}
...
function inferNumericType(value, env) {
    ...
    if (value.includes('.') || value.includes('π'))
        return 'ℝ';
    if (names.some(name => env.vars.get(name) === 'ℝ' || env.vars.get(name) === 'Real'))
        return 'ℝ';
    return 'ℕ';
}
...
function unifyTypes(left, right) {
    ...
    if (right === 'Any')
        return left;
    if ((left === 'Nat' && right === 'Real') || (left === 'Real' && right === 'Nat'))
        return 'Real';
    return 'Any';
}
function isAssignable(actual, expected) {
    ...
    if (normalizedActual === normalizedExpected)
        return true;
    if (isTypeVariable(normalizedExpected) || isTypeVariable(normalizedActual))
        return true;
    const actualList = listElementType(normalizedActual);
    const expectedList = listElementType(normalizedExpected);
    if (actualList && expectedList)
        return isAssignable(actualList, expectedList);
    if (normalizedActual === 'ℕ' && normalizedExpected === 'ℝ')
        return true;
    if (normalizedExpected.startsWith('Option(') && normalizedActual === 'Option(Any)')
        return true;
    if (normalizedExpected.startsWith('Result(') && normalizedActual === 'Result(Any,Any)')
        return true;
    return false;
}
