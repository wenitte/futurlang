import { LogicExpr } from './exprParser';
...
export interface ParsedLine {
  type: 'TheoremDeclaration' | 'DefinitionDeclaration' | 'ProofDeclaration' | 'LemmaDeclaration' | 'assert' | 'let' | 'raw';
  content: string;
  name?: string;
  tokens: Token[];
  bodyContent?: string; // Added to store raw body content
}
...
export function lexFL(text: string): ParsedLine[] {
  ...
  let currentDeclaration: ParsedLine | null = null;
...
function tokenizeLine(originalLine: string): Token[] {
...
// Helper functions
function matchConnective(line: string, start: number): { symbol: string; name: string } | null {
...
function matchSetSymbol(line: string, start: number): string | null {
...
function readString(line: string, start: number): { value: string; nextIndex: number } {
  let end = start + 1;
...
function readIdentifier(line: string, start: number): { value: string; nextIndex: number } {
  let end = start;
...
function isPartOfAnySymbol(c: string): boolean {
  for (const arr of Object.values(LOGICAL_CONNECTIVES)) {
    for (const sym of arr) {
      if (sym.includes(c)) return true;
    }
  }
  for (const sym of SET_SYMBOLS) {
    if (sym.includes(c)) return true;
