...
export function parseLogicExpression(expr: string): LogicExpr {
    ...
    function eat(t?: string): string {
        const cur = peek();
        if (!cur) throw new Error(`Unexpected end of expression at ${index} (remaining tokens: ${tokens.slice(index).join(' ')})`);
        if (t && cur !== t) throw new Error(`Expected '${t}', got '${cur}' at ${index}  (remaining tokens: ${tokens.slice(index).join(' ')})`);
        index++;
        return cur;
    }
    ...
    function parseQuantifier(): LogicExpr {
      ...
      eat('∈');
      const setName = eat();
      while (peek() === ' ') eat(); // Consume spaces before colon
      eat(':');
